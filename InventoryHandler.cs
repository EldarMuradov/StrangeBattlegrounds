...
public class InventoryHandler : MonoBehaviour
{
    ...
    private IEnumerator CreateItemsRoutine(string jsonArrayString)
    {
        ...
        for (int i = 0; i < jsonArray.Count; i++)
        {
            ...
            GameObject item = Instantiate(Resources.Load("Prefabs/Menu/InvItem") as GameObject, List.transform.position, List.transform.rotation);
    ...
    {
        StartCoroutine(GetItemsIds(UserInfo.Instance.UserId, _createItemsCallback));
    }
    private IEnumerator GetItemsIds(string id,Action<string> callback)
    {
        WWWForm form = new WWWForm();
        form.AddField("userId", id);
        using (UnityWebRequest www = UnityWebRequest.Post("http://localhost/ServerPHPCodeBase/GetPlayerItemsIds.php", form))
        {
